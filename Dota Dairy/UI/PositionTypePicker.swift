...
struct PositionTypePicker: View {
    @Binding var dismis: Bool
    @State private var title: String = "Position"
    @State var confirm: (GamerPosition) -> Void
    var body: some View {
        ZStack {
            if dismis {
                GeometryReader(content: { geometry in
                    VStack {
                        ...
                        ForEach(GamerPosition.allCases, id: \.id) {type in
                            Button(action: {
                                confirm(type)
                                dismis.toggle()
                            }, label: {
...
#Preview {
    PositionTypePicker(dismis: .constant(false), confirm: {_ in})
}
